...
本文件是 `msgcodec` 包的**唯一权威约定**。实现须严格遵循本规范,测试将据此验收。

## 1. 概述

`msgcodec` 是一个动态消息编解码库:消息结构由配置文件定义(而非编译期写死的 Go 结构体),运行时按类型 ID 注册,支持字段级取值/赋值与 JSON / Protobuf 双编解码。
...
- `int32`/`int64`:按 64 位补码 varint 编码(负数符号扩展,占 10 字节)。
